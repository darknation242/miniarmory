...
class tooltip {
    ...
    function __construct($items,$item_instance,$guid,$data) {
		...
		if($r = $mysql->getRows("select * from `item_instance` where owner_guid = ?2 and guid IN (?4-1)",
				$this->owner_id,SQL_template(3),$item_list,'char')) {
			foreach($r as $row) {
	...
	function get_tooltip($id) {
	  ...
	  if($id==0) return ($_LANGUAGE->text['not_equipped']).'.';
	...
	function update_set($slot) {
		...
		foreach($this->items as $id) {
			    preg_match_all('/><!--si([0-9]+):'.$id.'-->/s', $this->html[$slot], $match, PREG_SET_ORDER);
				if(count($match)) $ct++;
	            $this->html[$slot] = preg_replace('/><!--si[0-9]+:'.$id.'-->/s', ' class="q6">', $this->html[$slot]);
		}
		$match = NULL;
